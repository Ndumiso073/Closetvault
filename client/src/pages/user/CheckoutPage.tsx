...
import {
  ShoppingBag, ChevronRight, Shield, Truck, RotateCcw,
...
// ── Mock order items ──────────────────────────────────────────────────────
const ORDER_ITEMS = [
  { cartId: 1, product: PRODUCTS[0],  size: "10",  qty: 1 },
  { cartId: 2, product: PRODUCTS[4],  size: "9.5", qty: 1 },
  { cartId: 3, product: PRODUCTS[11], size: "8",   qty: 2 },
];
...
export default function CheckoutPage() {
  ...
  // totals
  const subtotal     = ORDER_ITEMS.reduce((s,i) => s + i.product.price * i.qty, 0);
  ...
  const placeOrder = () => {
    ...
    setPlacing(true);
    setTimeout(() => { setPlacing(false); setStep("success"); }, 1800);
  };
    ...
    return (
      <>
        <style>{`
          ...
          .co-success-tag::before, .co-success-tag::after {
        ...
        <div className="co-success">
          ...
          <h1 className="co-success-title">
            ORDER<br /><span>PLACED.</span>
          </h1>
          <p className="co-success-sub">
            Your order has been confirmed and will be dispatched shortly.
            A confirmation email has been sent to <strong style={{color:"var(--white)"}}>{form.email || "your inbox"}</strong>.
          </p>
          <div className="co-order-num">
            ORDER <span>#CV-{Math.floor(Math.random() * 90000) + 10000}</span>
          </div>
          <div className="co-success-actions">
            <Link to="/vault" className="btn-primary">
              <Package size={14} /> View My Vault
            </Link>
    ...
    <>
      <style>{`
        ...
        .co-step {
          ...
          color: var(--dim); background: var(--gray);
        ...
        .co-step.done .co-step-num { background: var(--accent); color: var(--white); }
        .co-step.active .co-step-num { background: var(--accent); color: var(--white); }
        ...
        .co-del-opt {
          ...
          padding: 14px 16px; background: var(--mid);
        ...
        .co-card-visual::before {
          content: ''; position: absolute; inset: 0;
          background: radial-gradient(ellipse at 80% 20%, rgba(255,45,0,.08), transparent 60%);
        }
        .co-card-brand {
          font-family: 'Bebas Neue', sans-serif;
        ...
        .co-card-chip {
          width: 32px; height: 24px; background: rgba(255,200,0,.3);
          border: 1px solid rgba(255,200,0,.4); border-radius: 3px;
        ...
        .co-card-field-val {
          font-family: 'Barlow Condensed', sans-serif;
          font-size: 13px; font-weight: 600; letter-spacing: 2px; color: var(--white);
        }
        ...
        .co-btn-back {
          ...
          border: 1px solid rgba(255,255,255,.12); padding: 12px 24px; cursor: pointer;
        ...
        .co-btn-next.loading { opacity: .7; pointer-events: none; }
        ...
        .co-oi-img {
          width: 52px; height: 52px; flex-shrink: 0; overflow: hidden; background: var(--mid);
        }
        ...
        .co-line-val {
          font-family:'Bebas Neue',sans-serif;
          font-size:15px; letter-spacing:1px; color:var(--white);
        }
        .co-line-val.free { color:#22c55e; }
        .co-total {
          display:flex; align-items:baseline; justify-content:space-between; margin:14px 0 16px;
        }
        .co-total-lbl {
          font-family:'Bebas Neue',sans-serif; font-size:17px; letter-spacing:3px; color:var(--white);
        }
        .co-total-val {
          font-family:'Bebas Neue',sans-serif; font-size:30px; letter-spacing:1px; color:var(--white);
        }
                ...
                <div className="co-card-body">
                  ...
                  <div className="co-form-grid full">
                    <div className="co-field">
                      ...
                      <input
                        className={`co-input ${errors.number ? "err" : ""}`}
                  ...
                  <div className="co-form-grid">
                    <div className="co-field">
                      ...
                      <input
                        className={`co-input ${errors.expiry ? "err" : ""}`}
                        placeholder="MM/YY"
                        value={card.expiry}
                    ...
                    <div className="co-field">
                      ...
                      <input
                        className={`co-input ${errors.cvv ? "err" : ""}`}
                        placeholder="•••"
                        type="password"
                        value={card.cvv}
                  ...
                  <div className="co-nav">
                    ...
                    <button
                      className={`co-btn-next ${placing ? "loading" : ""}`}
                      onClick={placeOrder}
                    >
                      {placing ? (
                        <>
                        ...
                        <><Lock size={13} /> Place Order · ${total}</>
                      )}
                    </button>
                  </div>
                </div>
                ...
                {ORDER_ITEMS.map(item => (
                  <div key={item.cartId} className="co-oi">
                    ...
                    <div className="co-oi-info">
                      <div className="co-oi-name">{item.product.name}</div>
                      <div className="co-oi-meta">
                        {item.product.brand} · US {item.size} · Qty {item.qty}
                      </div>
                    </div>
